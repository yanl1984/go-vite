...
type NodeConfig struct {
	...
	canTransfer func(db vm_db.VmDb, tokenTypeId types.TokenTypeId, tokenAmount *big.Int) bool
		...
		nodeConfig = NodeConfig{
			...
			canTransfer: func(db vm_db.VmDb, tokenTypeId types.TokenTypeId, tokenAmount *big.Int) bool {
				if util.IsViteToken(tokenTypeId) {
					balance := big.NewInt(0).Set(tokenAmount)
				...
				util.DealWithErr(err)
				return tokenAmount.Cmp(amountB) <= 0
			},
		}
	}
...
func (vm *VM) RunV2(db vm_db.VmDb, block *ledger.AccountBlock, sendBlock *ledger.AccountBlock, status util.GlobalStatus) (vmAccountBlock *vm_db.VmAccountBlock, isRetry bool, err error) {
	...
	if blockCopy.IsSendBlock() {
		if blockCopy.BlockType == ledger.BlockTypeSendCreate {
			quotaTotal, err := quota.GetQuotaForBlock(
				db,
			...
			vmAccountBlock, err = vm.sendCreate(db, blockCopy, true, quotaTotal)
		...
		if blockCopy.BlockType == ledger.BlockTypeSendCall {
			...
			vmAccountBlock, err = vm.sendCall(db, blockCopy, true, quotaTotal)
			return vmAccountBlock, noRetry, err
		}
	} else {
...
func (vm *VM) sendCreate(db vm_db.VmDb, block *ledger.AccountBlock, useQuota bool, quotaTotal uint64) (*vm_db.VmAccountBlock, error) {
	...
	// Check balance.
	if !nodeConfig.canTransfer(db, block.TokenId, block.Amount) {
	...
	qStakeUsed, qUsed := util.CalcQuotaUsed(useQuota, quotaTotal, quotaLeft, nil)
...
func (vm *VM) sendCall(db vm_db.VmDb, block *ledger.AccountBlock, useQuota bool, quotaTotal uint64) (*vm_db.VmAccountBlock, error) {
...
func (vm *VM) receiveCall(db vm_db.VmDb, block *ledger.AccountBlock, sendBlock *ledger.AccountBlock, meta *ledger.ContractMeta) (*vm_db.VmAccountBlock, bool, error) {
	...
	if meta == nil {
		qStakeUsed, qUsed := util.CalcQuotaUsed(true, quotaTotal, quotaLeft, nil)
		vm.updateBlock(db, block, nil, qStakeUsed, qUsed)
	...
	_, err = c.run(vm)
	if err == nil {
		qStakeUsed, qUsed := util.CalcQuotaUsed(true, quotaTotal, c.quotaLeft, nil)
	...
	if err == util.ErrOutOfQuota {
		...
		qStakeUsed, qUsed := util.CalcQuotaUsed(true, quotaTotal, c.quotaLeft, err)
		vm.updateBlock(db, block, err, qStakeUsed, qUsed)
		if refundFlag {
			var refundErr error
			if db, refundErr = vm.doSendBlockList(db); refundErr != nil {
...
func doRefund(vm *VM, db vm_db.VmDb, block *ledger.AccountBlock, sendBlock *ledger.AccountBlock, refundData []byte, needRefund bool, refundBlockType byte) bool {
	refundFlag := false
	if sendBlock.Amount.Sign() > 0 && sendBlock.TokenId == ledger.ViteTokenId {
		refundAmount := new(big.Int).Set(sendBlock.Amount)
		vm.vmContext.AppendBlock(
			util.MakeRequestBlock(
				block.AccountAddress,
				sendBlock.AccountAddress,
...
func (vm *VM) doSendBlockList(db vm_db.VmDb) (newDb vm_db.VmDb, err error) {
	...
	for i, block := range vm.sendBlockList {
		...
		case ledger.BlockTypeSendCall:
			sendBlock, err = vm.sendCall(db, block, false, 0)
			if err != nil {
				return db, err
			}
